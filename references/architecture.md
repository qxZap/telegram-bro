...
Scripts connect to Convex via HTTP using the deployment URL stored in the config file (captured from `npx convex function-spec` at setup time — the deploy key alone does not encode the region segment of the URL).

### Configuration

Stored at `~/.claude-telegram/config.json` (OS home dir, cross-platform — `%USERPROFILE%\.claude-telegram\config.json` on Windows):

```json
{
  "botToken": "123456:ABC...",
  "userId": "123456789",
  "deployKey": "prod:deployment|key...",
  "convexUrl": "https://deployment.eu-west-1.convex.cloud",
  "setupDate": "2026-04-19T17:43:00.469Z"
}
```

Lives outside the skill directory so the skill can be reinstalled or moved without losing credentials, and so secrets never enter a repo.
