...
```bash
tsx <SKILL_DIR>/scripts/setup.ts <bot_token> <user_id> <deploy_key>
```

Where `<SKILL_DIR>` is the absolute path where this skill is installed. The setup script `chdir`s to the skill directory internally, so it is safe to call from any cwd.
...
If the setup script doesn't work, you can set up manually. All paths below assume you are in the skill directory (`cd <SKILL_DIR>`):

```bash
# 1. Install dependencies
npm install

# 2. Create user-level config file (home dir, OS-agnostic)
#    Linux/macOS: ~/.claude-telegram/config.json
#    Windows:     %USERPROFILE%\.claude-telegram\config.json
mkdir -p "$HOME/.claude-telegram"
cat > "$HOME/.claude-telegram/config.json" << EOF
{
...
# 3. Create .env.local inside the skill dir (for `npx convex` commands)
echo "CONVEX_DEPLOY_KEY=YOUR_DEPLOY_KEY" > .env.local

# 4. Deploy to Convex
npx convex deploy

# 5. Capture the real deployment URL (includes region segment) and
#    merge it into the config. The deploy key alone is NOT enough —
#    without this step, scripts will 404.
URL=$(npx convex function-spec | node -e "let d=''; process.stdin.on('data',c=>d+=c).on('end',()=>console.log(JSON.parse(d).url))")
node -e "const fs=require('fs'),p=process.env.HOME+'/.claude-telegram/config.json';const c=JSON.parse(fs.readFileSync(p));c.convexUrl=process.argv[1];fs.writeFileSync(p,JSON.stringify(c,null,2))" "$URL"
