...
export default defineSchema({
  ...
  // Message history table — both outbound (Claude → user) and inbound
  // (user → bot via Telegram webhook).
  message_history: defineTable({
    title: v.string(),
    message_text: v.optional(v.string()),
    sent_at: v.number(),
    status: v.string(), // "sent", "failed", "received"
    direction: v.optional(v.string()), // "outbound" (default) or "inbound"
    error_message: v.optional(v.string()),
    scheduled_message_id: v.optional(v.id("scheduled_messages")),
    telegram_message_id: v.optional(v.number()), // Telegram's own id for both directions
    reply_to_telegram_message_id: v.optional(v.number()), // set on inbound when user replies to an existing message
  })
