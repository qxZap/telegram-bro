...
export const sendMessage = action({
  ...
  handler: async (ctx, args) => {
    ...
    const titleFromText =
      args.message_text.length > 60
        ? args.message_text.slice(0, 57) + "..."
        : args.message_text;

    try {
      let sentResult: { message_id: number };
      if (args.storage_id) {
      ...
      await ctx.runMutation(internal.messages.addToHistory, {
        ...
        status: "sent",
        direction: "outbound",
        telegram_message_id: sentResult.message_id,
      });

      return { success: true, message_id: sentResult.message_id };
    } catch (error: any) {
      await ctx.runMutation(internal.messages.addToHistory, {
        title: titleFromText,
        message_text: args.message_text,
...
// Helper: Send text-only message. Returns the Telegram message_id of the
// posted message so the caller can persist it (used to match inbound
// replies back to a specific outbound message).
